...
def main():
    ...
    swarm.allcfs.create_subscription(Twist, '/cf231/cmd_vel', cmd_vel_callback, 10)
    ...
    while rclpy.ok():
        ...
        if received_first_cmd_vel and has_taken_off:
            if msg_cmd_vel.linear.z >= 0:
                cf.cmdPosition(
                    (cf.cmdPositionMsg.x, cf.cmdPositionMsg.y, cf.cmdPositionMsg.z),
                    yaw=cf.cmdPositionMsg.yaw + 5
                )
            else:
