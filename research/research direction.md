...
<!-- more papers -->
<!-- this one generates different obstacle configurations to see when the drone struggles -->
[5] [Constraint-Aware AI-Guided Seed Generation and Mutation for CPS-UAV System Testing](https://dl.acm.org/doi/10.1145/3786155.3795703)

<!-- this one adds a light-weight onboard module for drone security -->

[6] [A Security Coprocessor-Integrated Platform for Autonomous UAV Security](https://dl.acm.org/doi/10.1145/3762642)

<!-- survey about function related safety (ISO standard), FTA (fault-tree analysis) and AV risk assessments (testing) -->

[7] [A Survey on Enhancing Autonomous Vehicle Safety through Fault Tree Analysis and SOTIF Integration](https://dl.acm.org/doi/10.1145/3787221)
...
2. Lack of Benchmarks and 5. Need of Cost-Effective Solutions
...
### idea 3. accessible FTA module for UAV functionality testing

Ground-up: FTA, Crazyflie, accessibility

Validation of accessibility - the entire testing setup (internal FTA + external E2E) can cost less than $40, which is a bit higher than the average extension deck on the Bitcraze website.

Validation of testing - for each common task:
        localisation, mapping, search, transport, swarm formation, racing
    construct a set of hardware and software requirements
    and design a template group of tests

    for each abstract test for each task, implement a real test scenario that can be executed for the Crazyflie platform. 

    The accessible FTA module should provide qualitative analysis (logs).

    The testing platform (for black-box testing) should agree with internal FTA module for over 95% of test cases.  

Taking advantage of:
    - safety & testing are increased in high-risk systems such as self-driving cars
    - copying and making those techniques more accessible for all ACPS, could increase general ACPS safety? FTA + SOTIF seen for UAV? FTA is seen for UAV, but not with SOTIF (which is a form of E2E).

Value Add:

Proof that FTA and E2E can be applied to nano-UAVs for testing and analysis and from a resource perspective logic, most ACPS.

a combination of both internal analysis and external testing can increase confidence about a system's functional quality.  
...
> ground-up approach rather than top down i.e. Crazyflie platform + Common Drone Goals
